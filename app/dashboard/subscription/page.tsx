      ...
      {isActive && (
        <div className="glass-card" style={{ padding: 28, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'rgba(74,222,128,0.08)', borderRadius: 10, marginTop: 4 }}>
              <div style={{ fontWeight: 600 }}>Total Monthly</div>
              <div style={{ fontWeight: 700, color: 'var(--accent-green)', fontSize: '1.1rem' }}>{formatCurrency(monthlyPrice)}</div>
            </div>
          </div>
        ...
        <div className="glass-card" style={{ padding: 28 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {[
              ...
              <Link key={i} href={`/subscribe?plan=${p.plan}`} style={{ textDecoration: 'none' }}>
                <div className="glass-card" style={{ padding: 20, border: p.recommended ? '1px solid rgba(74,222,128,0.3)' : '1px solid var(--border-subtle)', cursor: 'pointer' }}>
                  {p.recommended && <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--accent-green)', marginBottom: 8, textTransform: 'uppercase' }}>Best Value</div>}
                  <div style={{ fontWeight: 700, fontSize: '1.2rem', marginBottom: 4 }}>{p.price}</div>
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 12 }}>{p.label}</div>
                  {p.features.map((f, j) => <div key={j} style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: 4 }}>✓ {f}</div>)}
                </div>
              </Link>
            ))}
          </div>
